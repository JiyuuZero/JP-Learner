// validate.mjs — refuse to trust any class JSON on any failure.
// Shape: ajv against the FROZEN content.schema.json.
// Semantics: the 3 cross-field renderability invariants + ID determinism,
// enforced on vocab items AND on any sentence (example / grammar example)
// that carries a tokens[] array (word-bank chips must reconstruct the sentence).
...
// CLI entry: node validate.mjs <path-to-class.json>
if (process.argv[1] && process.argv[1].endsWith('validate.mjs')) {
  const file = process.argv[2];
  if (!file) {
    console.error('usage: node validate.mjs <class.json>');
    process.exit(1);
  }
  try {
